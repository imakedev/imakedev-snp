...
<form class="form-horizontal"  style="border:1px solid #B3D2EE;background: #F9F9F9;padding-top:20px;padding: 20px" action="" method="post" >
   ...
   <table>
   	...
   	<tr>
   		...
   		<td><input type="text" id="actual_vs_target_input" class="input_number" readonly="readonly" /> </td> 
   	</tr>
   	<tr>
   		<td>Weight:</td>
     	...
     	"<tbody>";
				...
				for(var i=0;i<data.length;i++){  
	            	...
	            	"<td style=\"text-align: center;\">"+
	          	    ...
	          	    " <i title=\"Edit\" onclick=\"showForm('edit','"+data[i][0]+"','"+data[i][1]+"','"+data[i][3]+"','"+data[i][5]+"')\" style=\"cursor: pointer;\" class=\"icon-edit\"></i>&nbsp;&nbsp;"+
	          		"</td>"+
	          		"</tr>  ";
				}
				/* str=str+"</tbody> </table>";
...
10. เมื่อทำการบันทึกข้อมูล ให้ระบบเซ็ตค่า field approved_flag = 'N' */
				...
				if(data[0][14]!=null && data[0][15]!='null'){ 
					$("#performance_percentage_input_hidden").val(data[0][15]);
				}else
							    	 ...
							    	 KPIAjax.searchObject(query2,{ 
							    				...
							    				//	alert(data2); 
						    						...
						    						$("#performance_percentage_input_hidden").val(performance_percentage_value);
							    						var weight_percentage_value=parseFloat((parseFloat($("#weight_input").val()).toFixed(2))*(performance_percentage_value)).toFixed(2);
...
from kpi_baseline
where kpi_code = ${kpi_code} ของ record ที่แก้ไขข้อมูล
and ${baseline_data} between begin_baselien and end_baseline  
% Actual vs Target = (Actual Score*100)/Target Score ได้เกิน 100% ก็ปรับให้เหลือ 100%
Weight Percentage = KPI Weighting * Performance Percentage
10. เมื่อทำการบันทึกข้อมูล ให้ระบบเซ็ตค่า field approved_flag = 'N'
*/
// 	var mode=$("#mode").val();
	...
	var query="";
	  //edit
		  ...
		  ",performance_percentage="+$("#performance_percentage_input_hidden").val()+ 
